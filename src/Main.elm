...
viewBody model =
    let
        palette =
            getPalette model
    in
    layout [ Font.family [ Font.external{ name = "Space Mono", url = "https://fonts.googleapis.com/css2?family=Space+Mono"}, Font.sansSerif ], Background.color palette.outerBg, Font.color palette.defaultText ] <|
        column
            -- Card with shadow
            [ centerX
            , centerY
